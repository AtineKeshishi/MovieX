...
/**
 * The page source for getting the response and handle pagination of trending tv show list in
 * [HomeFragment]
 *
 * @property repository is the repository for getting the response from the server.
 */
class TrendingShowsPageSource(private val repository: NetworkRepositoryInterface) :
    PagingSource<Int, CombinedMovies>() {

    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, CombinedMovies> = try {
        val page = params.key ?: 1
        val response = repository.getTrendingShows(page)
