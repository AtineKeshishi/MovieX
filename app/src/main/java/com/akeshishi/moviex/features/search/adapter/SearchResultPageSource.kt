...
/**
 * The page source for getting the response and handle pagination of search result list in
 * [SearchFragment].
 *
 * @property query is the user's searched phrase.
...
class SearchResultPageSource(
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, CombinedMovies> = try {
        ...
        val response = repository.searchMulti(query, page)
        val previousPage = if (page - 1 > 0) page - 1 else null
        val nextPage = if (response.multiList.isNullOrEmpty()) null else page + 1
        LoadResult.Page(response.multiList, previousPage, nextPage)
